...
4. **timesfm / autots** (chỉ khi user đã cho phép cài, plan §2.2): pin `timesfm[torch]==2.0.2`, `autots==1.0.4 + statsmodels`. Rủi ro đã biết: `timesfm[xreg]` kéo `jax[cuda]` **xung đột wheel `nvidia-*` với torch cu128** → dùng `jax` bản CPU + `force_on_cpu=True`; autots chưa xác minh với pandas 3.0.3 → smoke import trước, **không tự hạ pandas**.
5. Sau khi sửa: chạy lại preflight + `python -m pytest -q`, ghi version thực tế vào `experiments/env.txt` và MEMORY (Data/Implementation Blockers).
