...
File này chỉ giữ invariants. Kế hoạch nghiên cứu: `docs/RESEARCH_PLAN.md`. Code: `src/p0/` + `run.py` (§8 plan); prompt Vast: `docs/VAST_SESSION_PROMPT.md`. Trạng thái hiện tại: `.claude/MEMORY.md` (auto-import cuối file). Bản cũ (2026-08-24) ở `docs/archive/` và tài liệu tham khảo ở `docs/reference/` không có hiệu lực. Agents: `.claude/AGENT.md`.
...
- Nguồn: `data\BTC_hf_1min.csv` (+ `data\BTC_lf_5min.csv` cho feature 5 phút); cột `datetime,timestamp,open,high,low,close,volume,amount`; UTC, lưới 60 s. `data\` read-only; không trộn `*_close.csv`; không tự fetch data mới khi user chưa yêu cầu. Snapshot đang dùng có sha256 trong `data/data_checksums.json` (§6.1) — CLI từ chối chạy khi CSV không khớp; không ghi đè file này khi chưa có lệnh user.
...
- **B0 = `Baseline_LGBM.py`** frozen (settings deny Edit/Write): không sửa file, không đổi hyperparameter. Bug đã biết: `TargetTransform` của B0 nhân in-place `(n,1) *= (1,3)` → ValueError; harness dùng `src/p0/transform.py` tái hiện đúng công thức, phần còn lại import từ B0. 306 feature của B0 được **lọc nhiễu một lần** (plan §1.4: permutation importance + standalone 1-feature vs E0/B0-306 + mutual information; kiểm chứng bằng 3 run so với B0-306) thành **B0\*** = feature baseline chung; lọc bằng chọn cột trong harness, B0-306 vẫn log làm reference; candidate = B0\* + cột ext.
- **Model** (thứ tự chạy = thời gian chạy tăng dần): LightGBM → XGBoost → CatBoost → TimesFM (**hai nhánh** `tfm_b0` từ B0\* và `tfm_ext` từ ∅/native, mỗi nhánh đủ add-one → prune → confirmation, rồi `tfm-final` chọn TimesFM-final) → XGB-RF (XGBoost random-forest mode trên GPU, thay ExtraTrees) → AutoTS (WR/MR cố định = **probe** dò feature → F_WR_best/F_MR_best → `autots-search` chạy framework riêng cho từng bộ → AutoTS-final) → LSTM-DMH (cuối); ensemble của các model tốt. **Mọi model xuất phát từ cùng B0\***; mỗi model calibrate riêng một run ES trên B0\* bằng `calib_seed` → `15fixed_m` (tree) / `fixed_epoch_LSTM` (LSTM) — XGB-RF/TimesFM/AutoTS theo cơ chế riêng, không ép fixed_rounds — rồi tự chạy vòng lặp add-one (KEEP nếu tốt hơn/không đổi, DROP nếu tệ hơn ε_m) → F\*_m riêng (F\*_LGBM, F\*_XGB, … có thể khác nhau) → prune PI (bỏ cột ext có PI ≤ 0; không safety-net) → 3 seed, mean RMSE từng ô → Gain 15 ô prune vs unprune → MedianGain ≥ −ε_m chọn prune → win_m; không model nào kế thừa F\* của model khác; số vòng không dùng chéo (`15fixed_306` chỉ cho lọc B0); cờ lọc B0 = > 0 ở ≥ 2/3 horizon; chạy từng model một; sau mỗi model so với champion (ban đầu = LightGBM code gốc) và log đổi/giữ. Không thêm model chỉ để dài danh sách; không AutoTS tự search.
- **Metric**: model dự báo log return ŷ_h, nhưng **metric tính trên giá** `P̂ = C_t·exp(ŷ_h)` (USD): RMSE, MAE trên `P̂_{t+h} − C_{t+h}`; `Gain = 1 − RMSE_cand/RMSE_base` per horizon × fold; MedianGain, WinRate, P10Gain, WorstGain; Pearson r và directional accuracy trên thay đổi giá `P̂ − C_t` vs `C_{t+h} − C_t`; importance chỉ là diagnostic; E0 (P̂ = C_t) luôn log. Không thêm metric mới. Ba vai trò seed tách bạch (§1.3): `calib_seed` chỉ để ES lấy số vòng cố định; 3 `eval_seeds` để đo ε (mỗi ô: `noise = 100·std/mean` của 3 RMSE, ε = max(floor, RMS 15 ô), không seed nào làm mốc) và confirmation; **một `selection_seed` duy nhất** cho mọi bước selection (R1–R4, baseline + 39 candidate, prune PI, Final). **Chỉ MedianGain (với ε) quyết định** KEEP/DROP, chọn B0*, champion, thành viên ensemble; WinRate/P10/Worst chỉ báo cáo; PI/MI/standalone chỉ để lập R1–R4 khi lọc B0. Inference latency (predict một origin, batch 1, p95/p99/max per model × horizon) chỉ để theo dõi — không ảnh hưởng training/loss/quyết định. Log đầy đủ mỗi candidate và mỗi lần đổi/giữ champion; bảng tổng hợp mọi model; visualize sau mỗi model = win vs champion (mỗi horizon 3 ảnh theo origin ở 3 ngày VAL vol thấp/trung bình/cao + 2 heatmap), Final = heatmap mọi model + ảnh theo horizon với prediction mọi model; actual luôn đen, màu phân biệt rõ, không vẽ chuỗi dự báo liên tục.
