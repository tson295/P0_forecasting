...
đáp ứng phương pháp cố định. Không train. Không smoke/canary/test/trial fit.
...
- Replay v2 (`src_OB/reconstruct.py`, `REPLAY_VERSION = 2`): buffer bridge trước snapshot, không neo lại snapshot cũ khi
  ...
  gap theo `known_hard_gaps_utc` trong config và không bridge qua hard gap. **Chưa có prepare thật nào dùng v2**; prepared HF
  v3 giữ nguyên (checker lượt 2–3 suy luận từ evidence, chưa chạy thật: trên HF v2 chỉ đổi nhãn reset). Checker lượt 2, 3
  và cuối run: không ERROR. Còn mở trước prepare v2 thật đầu tiên: R3-I1, R3-I2, R3-I4, `reset_after` của R3-I5 và F-02
  (ghi SHA code/hash config vào manifest hoặc tăng `REPLAY_VERSION`) — xem CHECKER_FINDINGS.
...
Đóng R3-I1/I2/I4/I5 + F-02 → config riêng cho nguồn mới (`dataset_*`, `raw_dir`, `prepared_dir`, `output_dir` mới;
