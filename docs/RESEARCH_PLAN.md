...
1. `bash scripts/vast_bootstrap.sh` → `scripts/vast_canary.py` + `scripts/canary_xreg_gpu.py` → (nếu chưa scp LF) `python run.py derive-lf --config configs/p0_full.json` (phải ra sha `0e5fb9ad…`) → `python run.py check-data --config configs/p0_full.json` (verify anchor `data/data_checksums_2y.json` đã commit; in 5 fold `rolling_spread` + final như bảng §1.5) → `python run.py lock-s0 --config configs/p0_full.json` (S0_m, Candidate_m = 163, `s0/collisions.json`, audit label = dataset 2 năm) → `pytest -q -x` → `scripts/checker_record.py --exp experiments/full --blocking` sạch ERROR.
