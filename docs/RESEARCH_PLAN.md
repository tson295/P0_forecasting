...
| `selection_seed` = 8587 (mặc định = `eval_seeds[0]`) | **một giá trị duy nhất** cho MỌI bước selection: PI/SA/MI và 4 run R1–R4 (phase A); baseline B0\* + toàn bộ 39 candidate add-one + prune PI (phase B); refit Final | không được đổi seed giữa các Rk hoặc giữa các candidate |
...
Trước vòng lặp: calibrate phase B của `m` trên B0\* (§1.3) → `15fixed_m` (LightGBM/XGBoost/CatBoost) hoặc `fixed_epoch_LSTM` (LSTM) bằng `calib_seed`, và ε_m từ 3 evaluation seed; XGB-RF/AutoTS/TimesFM không có gì để calibrate ngoài ε_m. **Run baseline B0\* và toàn bộ 39 candidate đều chạy ở `selection_seed`** (một giá trị duy nhất) với số vòng cố định.

`S_m := B0*`. Với từng feature `f` trong §2.3, theo đúng thứ tự:

1. Input = B0\* + các cột ext đang KEEP của `m` + `f` (giá trị tại origin t, lag 0; với LSTM là chuỗi theo phút của cùng cột). **Ngoại lệ TimesFM**: baseline không phải B0\* mà là chiến lược đã freeze ở §2.2 #4b (đã chốt `ext_only`: backbone r1, covariate = các candidate ext đang KEEP + `f`, KHÔNG có cột B0\*).
...
| 2 | XGBoost | `hist`, `device=cuda`, `reg:pseudohubererror` (huber_slope 0.9), lr 0.03, max_depth 6, seed theo vai trò §1.3; cùng TargetTransform | §2.1 từ B0\* với `15fixed_XGB` → F\*_XGB | **≈ 1–2 h** |
| 3 | CatBoost | GPU, `Huber:delta=0.9`, lr 0.03, depth 6, seed theo vai trò §1.3; cùng TargetTransform | §2.1 từ B0\* với `15fixed_Cat` → F\*_Cat | **≈ 1–2 h** |
| 4 | TimesFM | zero-shot, không train; input chuỗi r1 kết thúc tại t, context 512 (hoặc tối đa API); dự báo `r̂_{t+1..t+3}` → cộng dồn → giá; pin package/checkpoint/backend; tắt tùy chọn ép dương nếu API có. **Audit chốt** (`docs/reference/audit_timesfm.md`): `timesfm[torch]==2.0.2`, checkpoint `google/timesfm-2.5-200m-pytorch` rev `1d952420…`; `forecast(horizon=3, inputs=[...])` batch nhiều origin; `infer_is_positive=False`; dùng head **mean** `quantile[...,0]` (`point_forecast` là q50). timesfm 1.x không cài được trên Python 3.12 | (a) **TFM-POINT** không feature = baseline của model; (b) API **có** `forecast_with_covariates` (cần `timesfm[xreg]`: jax + sklearn): §2.1 với candidate §2.3 làm covariate theo phút (xuất phát không covariate), giá trị cho 3 bước dự báo = giữ giá trị tại t → F\*_TFM. Ba ràng buộc bắt buộc từ audit: **1 origin mỗi lời gọi** (xreg fit một `beta_hat` chung cho cả batch → gộp origin là leakage §6.4), covariate **dịch 1 bar** (vị trí s mang f(s−1)), và đường covariate phải compile riêng `per_core_batch_size=1` + dùng **cùng head mean** với đường point. **Chiến lược baseline covariate đã CHỐT trước run (audit §12, 2026-08-31): `ext_only`** — backbone r1 native, covariate chỉ gồm candidate §2.3, KHÔNG dùng B0\*. Căn cứ: hồi quy in-context của xreg chỉ có **512 hàng**, mỗi covariate = 1 cột (+intercept) và `ridge=0` ⇒ n/p = 12.8 (k=39) / 5.07 (k=100) / 1.67 (k=306); phần đóng góp xreg là hằng số c cho cả 3 bước nên khi feature vô dụng, RMSE/E0 = `sqrt(1+h·L)` → sai số cộng thêm **+2.9…4.2 pp (h=1) ở k=39** và **+10…59 pp ở k=306**, trong khi tín hiệu 1 phút khả dĩ chỉ 0.1–0.2 pp ⇒ dùng cả B0\* làm covariate **chắc chắn thua E0**. Thêm: float32 + padding lũy thừa 2 làm rank hiệu dụng chỉ còn 71/307 ở k=306 (đo ngưỡng số học chứ không đo feature), và chi phí 40 run là 1.2 h (`ext_only`) vs 25–31 h (`b0star_full`, GPU idle ~97%). Vòng add-one giữ k nhỏ nên n/p luôn lành mạnh. Chiến lược được ghi vào `experiments/tfm_strategy.json` và **freeze**: đổi giữa pipeline = CLI dừng; (c) nếu (a)/(b) thắng E0 (MedianGain > 0 vs E0) → **TFM-LoRA**: rank 8 trên attention/FF, train FIT, ES trên ES set, Huber trên `r̂_{t+1..t+3}`, 1 config, 3 seed | inference batch 7.2k origin ≈ 1–2 phút/run → TFM-POINT vài phút; loop covariate **≈ 2–6 h** (audit: batch = 1 nên đắt hơn ước tính cũ 1–1.5 h; đo 1 fold trước khi cam kết); LoRA (nếu chạy) ≈ 1–2 h qua `transformers>=5.4` + `peft` (package `timesfm` không có API LoRA) |
| 5 | XGB-RF (thay ExtraTrees) | XGBoost random-forest mode trên GPU: `num_parallel_tree=500`, `subsample=0.63`, `colsample_bynode=0.3`, `learning_rate=1`, 1 vòng boosting, `max_depth=8`, `min_child_weight=500`, squared error trên z-target, `device=cuda`, seed theo vai trò §1.3 | §2.1 từ B0\* (1 vòng boosting cố định — không có gì để calibrate ngoài ε) → F\*_XGBRF | 15 fit × ~10 s ≈ 2–3 phút/candidate → **≈ 1.5–2 h** |
| 6 | AutoTS — 2 model cố định | `WindowRegression` (regression_model LightGBM, GPU) và `MultivariateRegression` (regression_model XGBoost, GPU). **Audit chốt** (`docs/reference/audit_autots.md`): `autots==1.0.4`, import `autots.models.sklearn`, gọi thẳng class (không dùng `AutoTS(...)` = search), `regression_model={"model": "LightGBM"|"xgboost", "model_params": {...GPU...}}`, rolling `fit` 1 lần/fold + `fit_data`/`predict` mỗi origin (không refit); regressor **dịch theo model** (MR: `R.loc[s]=f(s−1)`; WR: `R.loc[s]=f(s+window−1)`, predict truyền `f(t)`), vá bug `sklearn.py:3337` ở phía ta; chấm trên **toàn bộ origin** như mọi model (§6.6 — bỏ lưới thưa 5' để §3 so cùng 15 ô); target r1, `forecast_length = 3` → cộng dồn → giá; base regressor = B0\*; transformer cố định tối thiểu, không search, không ensemble nội bộ; số vòng của regression_model bên trong cố định trong config theo cơ chế AutoTS (không ép fixed_rounds) | **Ba giai đoạn (rev 2026-08-31, user chốt)**. (i) **Probe**: WR và MR cố định, mỗi cái một vòng lặp §2.1 từ B0\* với candidate làm regressor → **F_WR**, **F_MR**. Probe CHỈ để dò feature — không so champion, không vào ensemble, không refit ở Final. (ii) **Union** (`autots-union`, chỉ sau khi cả hai probe xong): mỗi probe chạy thêm đúng 1 run với `F_union = F_WR ∪ F_MR`, chọn bộ tốt hơn giữa {riêng, tổng hợp} theo MedianGain → **F_WR_best**, **F_MR_best**, rồi **FREEZE**. (iii) **Bake-off AutoTS** (`autots-search`): với mỗi frozen set (dedup nếu hai bộ trùng), mỗi fold — nạp `initial_template` do ta khai báo (mọi dòng ép GPU), `max_generations=0`, `model_list=['WindowRegression','MultivariateRegression']`, `transformer_list=[]`, `regression_type='User'`, `datepart_method=None`, `holiday=False`; AutoTS chạy validation nội bộ **chỉ trên training-side của fold** (FIT+ES, kết thúc trước purge 60') và chọn best template → **freeze template** → rolling predict outer VAL bằng `ModelMonster` (không refit). Feature set TUYỆT ĐỐI không đổi trong bake-off (assert `regression_type=='User'`, `datepart_method is None`, `holiday is False`, không transformer — template vi phạm bị loại). Chấm hai frozen set bằng **metric của project** (`P̂ = C_t·exp(ŷ_h)` → RMSE/Gain 15 ô → MedianGain), KHÔNG dùng điểm nội bộ của AutoTS làm kết luận → bộ tốt hơn là **AutoTS-final**, mới đi so champion (§3) và vào ensemble | rolling-origin trên lưới thưa (mỗi 5 phút, ~1.4k origin) ≈ 3–5 phút/candidate → **≈ 2–4 h/model, 4–8 h cả hai**; run cuối chấm trên toàn bộ origin |
...
**Code đã viết (2026-08-29): 87 unit test PASS (CPU, data tổng hợp), smoke end-to-end PASS, checker review đã sửa (gate, checksum §6.1, schema log); đủ 8 model của §2.2.** `src/p0/`: `data` (adapter, kiểm tra §1.1 gồm gap/dup, checksum §6.1, as-of LF), `split` (§1.2), `features_ext` (39 cột §2.3, causal), `metrics` (trên giá, Gain, tóm tắt, gộp 3 seed), `transform` (tái hiện `TargetTransform` của B0 — **B0 gốc có bug nhân in-place `(n,1) *= (1,3)` trong `TargetTransform` nên không chạy được nguyên bản; file B0 không sửa, công thức giữ nguyên**), `models` (LightGBM đúng code gốc B0 qua `_make_model`/`LGBMConfig`, XGBoost, XGB-RF mode `num_parallel_tree`, CatBoost; `models_lstm`; `models_tfm` = TimesFM 2.5 zero-shot + covariate; `models_autots` = WindowRegression/MultivariateRegression — hai cái sau theo `docs/reference/audit_*.md`, **chưa cài package** nên mới test bằng stub), `harness` (Store = ma trận B0 + ext, `run_config`, `calibrate` → 15fixed_m, `seed_noise` → ε_m), `filter_b0` (§1.4), `loop` (§2.1 add-one, prune PI, 3 seed → win_m; §3 champion, ensemble), `latency` (§7.4), `plots` (§7.3), `logs` (§7: `log.csv`, `champion_log.csv`, `summary/latency_summary.csv` schema cố định; `runs/<exp_id>/`), `cli`. Config: `configs/p0_15d.json`. Test: `python -m pytest -q`; smoke: `python run.py smoke-e2e --out tmp_smoke --days 6` (data tổng hợp, CPU, chỉ debug). Prompt session Vast: `docs/VAST_SESSION_PROMPT.md`; bootstrap: `scripts/vast_bootstrap.sh`; môi trường: `requirements.txt`.
...
3. `python run.py loop --model lgbm` → `xgb` → `cat` → `tfm` → `xgbrf` → `autots_wr` → `autots_mr` → `lstm`: mỗi lệnh = calibrate riêng trên B0\* (15fixed_m / fixed_epoch, ε_m) → 39 candidate → prune PI → 3 seed (mean RMSE từng ô) → win_m → latency → champion log + Fig H_h/HM.
4. `python run.py autots-union` (§2.2 #6 ii: F_WR_best/F_MR_best, freeze) → `python run.py autots-search` (iii: bake-off template GPU → AutoTS-final) → `python run.py ensemble` (§3) → `python run.py final` (§4: TEST một lần → `summary/all_models_test.csv` gồm Gain vs E0/B0-306/B0\*/champion + latency, heatmap + Fig H_h mọi model, `latency_summary.csv`).
...
- AutoTS: WR/MR cố định = **probe dò feature** (riêng/tổng hợp sau khi cả hai vòng lặp xong) → freeze F_WR_best/F_MR_best → **bake-off template GPU** (`max_generations=0`) chọn AutoTS-final; genetic self-search bỏ vì không thể ép GPU-only (§9).
